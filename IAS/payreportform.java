...
import java.sql.*;
import java.awt.*;
public class payreportform extends JFrame implements ActionListener,FocusListener
{
    JLabel lblcolor =new JLabel();
    JLabel lblcolor2 =new JLabel();
    JLabel lblimg = new JLabel();
    JLabel lblname=new JLabel("Payment Reoprt Form");
    JLabel lblname2=new JLabel("Payment Table");
...
public void focusLost(FocusEvent f)
{
        ...
        if(a.getSource()==btnprint)
    {
        try{
            ...
            while(res.next())
            {
                 pw.printf("%-15s",res.getString(1));
                 pw.printf("%-15s",res.getString(2));
                 pw.printf("%-15s",res.getString(5));
                 pw.printf("%-15s",res.getString(7));
            }
            pw.close();
