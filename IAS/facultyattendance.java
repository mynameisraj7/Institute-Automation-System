...
import java.util.*;
import java.awt.*;
public class facultyattendance extends JFrame implements ActionListener,KeyListener,FocusListener
{
    JLabel lblcolor =new JLabel();
    JLabel lblcolor2 =new JLabel();
    JLabel lblcolor3 =new JLabel();
    JLabel lblimg = new JLabel();
    JLabel lblname=new JLabel("Faculty Attendance Form");
    JLabel lblname2=new JLabel("Faculty Attendance Table");
    Date first;
    JLabel lblstart=new JLabel("Start Date");
    JTextField txtstart=new JTextField();
    @SuppressWarnings("rawtypes")
    JComboBox cmbbid=new JComboBox();
    String col[]={"S_No","Name","Faculty Id","Attendance"};
    ...
    public void focusLost(FocusEvent f)
    {
        ...
        if(f.getSource()==txtstart)
        {
            ...
            try
            {
                ...
                ResultSet res=st.executeQuery("select * from facultyattendancetbl where date='"+txtstart.getText()+"'");
                model.setRowCount(0);
                while(res.next())
                {
                    @SuppressWarnings("rawtypes")
    ...
    public void actionPerformed(ActionEvent a)
    {
        ...
        if(a.getSource()==btnsave)
        {
            try
            {
                ...
                while(res.next())
                {
                    st.executeUpdate("delete from facultyattendancetbl date='"+txtstart.getText()+"'");
                }
            }
            catch(Exception e)
            {
            ...
            try{
                ...
                {
                Connection con=DriverManager.getConnection("jdbc:mysql://localhost:3306/ias","root","mysqlstudent");
                String str="insert into facultyattendancetbl(fname,fid,date,attendance) values(?,?,?,?)";
                PreparedStatement ps=con.prepareStatement(str);
                for(int i=0;i<tbl.getRowCount();i++)
                {
                    ps.setString(1,""+tbl.getValueAt(i,1));
                    ps.setString(2,""+tbl.getValueAt(i,2));
                    ps.setString(3,txtstart.getText());
                    ps.setString(4,""+tbl.getValueAt(i,3));
                    ps.executeUpdate();
                }
