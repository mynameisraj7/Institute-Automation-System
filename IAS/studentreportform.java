...
import java.sql.*;
import java.awt.*;
public class studentreportform extends JFrame implements ActionListener,FocusListener
{
    JLabel lblcolor =new JLabel();
    JLabel lblcolor2 =new JLabel();
    JLabel lblimg = new JLabel();
    JLabel lblname=new JLabel("Student Report Form");
    JLabel lblname2=new JLabel("Student Attendance Table");
    JLabel lblstart=new JLabel("Start Date");
    JLabel lblend=new JLabel("End Date");
    JTextField txtstart=new JTextField();
    JTextField txtend=new JTextField();
    String col[]={"Student Id","Student Name","Date","Attendance"};
...
public void focusLost(FocusEvent f)
{
    if(f.getSource()==txtend)
    {
        try{
            ...
            ResultSet res=st.executeQuery("select * from attendancetbl where date between '"+txtstart.getText()+"' and '"+txtend.getText()+"'");
